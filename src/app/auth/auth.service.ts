...
export class AuthService extends IonicAuth {
  ...
  private async addConfig() {
    ...
    const redirectUri = this.onDevice() ? 'com.okta.dev-133320:/callback' : window.location.origin + '/callback';
    const logoutRedirectUri = this.onDevice() ? 'com.okta.dev-133320:/logout' : window.location.origin + '/logout';
    const clientId = '0oa50oyo7l3H4EjO9357';
